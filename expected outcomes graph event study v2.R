...
treated_effect_size = c(0.01, 0.015, 0, 0, -0.1, -0.09, -0.14, -0.17, -0.19)
...
ggplot(data, aes(x = time, y = effect_size, color = group)) +
  ...
  labs(x = "Years Before and After HU Initiation", y = "HU effect on hospital days per year",
       title = "Increasing Effect Size Over Time") +
  theme_classic() +
  scale_color_manual(values = c("blue", "red"), name = NULL) + 
  theme(legend.position = c(0.24, 0.24)) +
  scale_y_continuous(breaks = c(0, -0.1, -0.2))


# Create a data frame with time (years before and after HU initiation) and effect sizes
treated_effect_size = c(0.01, 0.015, 0, 0, -0.15, -0.14, -0.07, -0.06, -0.045)
untreated_effect_size = c(0.02, -0.01, 0, -0.01, 0.01, 0.02, 0.015, -0.005, 0)
...
ggplot(data, aes(x = time, y = effect_size, color = group)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Years Before and After HU Initiation", y = "HU effect on hospital days per year",
       title = "Decreasing Effect Size Over Time") +
  theme_classic() +
  scale_color_manual(values = c("blue", "red"), name = NULL) + 
  theme(legend.position = c(0.24, 0.24)) +
  scale_y_continuous(breaks = c(0, -0.1, -0.2))
